...
//定义DHT模块接入Esp8266的哪个引脚，本例以gpio5为例，大家可以自行更改与硬件连接一致。
#define DHT_PIN 5
...
//DHT模块初始化。第一个参数为模块类型，也就是DHT11或者22，
//第二个参数为读取温湿度值的间隔时间，单位为毫秒，根据模块说明采样周期不得低于1秒
void DHTInit(enum sensor_type sensor_type)
{
  SENSOR = sensor_type;
  // 设置GPIO5为标准的输入输出接口，如果大家使用别的GPIO接口，此语句也要修改为接口的接口
